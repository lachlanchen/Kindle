...
\begin{itemize}
  ...
  \item \textbf{仓库中的 PW5SE 自动启动已通过实机验收。} 本手册封面所写的 PW4 属于不同固件与安装体系，不要把 PW5SE 的 root 启动任务复制到 PW4。
\end{itemize}
\end{statusbox}
...
持续按住电源键，直到 Kindle 重新启动，通常可能需要约 40 秒。已验收的 PW5SE 会在重启后自动打开一次 KOReader；选择 \textbf{退出 KOReader} 后，本次开机不会再次拉起它。若下一次开机必须进入原生系统，请先按下文用“改名”方式激活恢复标记。
...
249 是带日期的初始快照，不是永久总数。Nutstore 后来新增一本文件名以 \texttt{Giving Up the Gun...} 开头、以“黑白.pdf”结尾的 LinguaLeaf PDF；当前目标语料库因此为 250 本（LinguaLeaf 240 本加 PocketPolished 10 本）。这本新书仍待传输，本文不声称已复制。
...
\path{/mnt/us/_DISABLE_KOREADER_AUTOSTART} & PW5SE 停放标记（普通文件）：自动启动已启用 \\
\path{/mnt/us/DISABLE_KOREADER_AUTOSTART} & PW5SE 生效标记（普通文件）：自动启动已禁用 \\
\bottomrule
\end{longtable}
...
PW5SE 使用“只改名、不删除”的恢复开关。在 Kindle 驱动器最外层，把以下停放的普通文件：
...
不要删除或自行新建标记。如果停放文件缺失，或两个名称同时存在，应停止并检查。安全弹出后拔掉 USB 数据线，再重启。重新启用必须使用审计过的管理器；它会先验证启动任务的精确 SHA-256，再反向改名。

\begin{warningbox}
PW5SE 已验收的启动任务 SHA-256 为 \path{2de0232b971926b7e70d913a27ba76168ed69760504ae2a90947e4402e7e5828}。拔掉 USB 数据线后的重启记录为 \texttt{started -> native-ready -> launching}；任务为 \texttt{start/running}，KOReader、Wi-Fi 与 SSH 均恢复。根文件系统为只读，\texttt{preventScreenSaver=0}。退出后任务为 \texttt{stop/waiting}，至少 45 秒没有阅读器进程，证明不会自动重启 KOReader。
\end{warningbox}
