...
Autostart must always be optional and device-specific. The PW5SE deployment uses exactly one regular file among three persistent mode names at the visible USB root. The PW4 named on this guide's cover is a separate firmware/package family; do not copy the PW5SE root job to it.

\begin{tabularx}{\textwidth}{>{\ttfamily}p{0.43\textwidth} X}
\toprule
\_DISABLE\_KOREADER\_AUTOSTART & Standard mode: KOReader starts ASAP while Amazon's framework remains resident. \\
\_DISABLE\_KOREADER\_AUTOSTART\_FRAMEWORK\_STOP & Lower-memory mode: Amazon's framework is stopped while KOReader runs and restored on clean exit. \\
DISABLE\_KOREADER\_AUTOSTART & Active regular file: autostart is disabled and the native interface remains. \\
\bottomrule
\end{tabularx}

The state change is rename-only. Disable by atomically renaming whichever underscore mode exists to the active name. Enable or select a mode only through the audited manager, which checks the exact installed job hash first. Missing, multiple, linked, directory, or special-file markers fail closed to native. The job gives the native system a recovery window, applies final safety gates, launches only once, and never respawns KOReader. Therefore choosing \menupath{Exit KOReader} returns to the normal interface for the rest of that boot.

\begin{warningbox}
\textbf{Current PW5SE boundary.} V1 hash \path{2de0232b971926b7e70d913a27ba76168ed69760504ae2a90947e4402e7e5828} passed boot, Wi-Fi/SSH return, clean exit, and no-respawn validation. It was atomically upgraded on 2026-08-13 to v2 hash \path{87381c8cb810b3e8606c97b5ad913a1be5f49c7a4ba6f46f66b6ae3e28e95dbd}; framework-stop is selected for the next boot. The root filesystem is read-only and \texttt{preventScreenSaver=0}. The first v2 reboot was deliberately not performed while the owner was reading, so do not confuse the accepted v1 result with v2 acceptance.
\end{warningbox}

Disconnect USB \textbf{data} before boot. No cable, a wall charger, or a charge-only cable is safe. A computer data cable can enter USB Drive Mode and export or make unavailable \path{/mnt/us}, where KOReader and its marker live.

\subsection{Brightness, warmth, and the freeze guard}

The 2026-08-13 audit found one concrete malformed-touch crash after wake and repeated low-memory cache eviction with a very large PDF. A reversible exact-hash gesture guard is installed for the next KOReader launch; lower-memory framework-stop is selected for the next boot. Amazon ambient auto brightness was also active underneath KOReader even though AutoWarmth was off. The managed patch now defaults it to manual and reapplies the choice after every resume.

Create the exact regular root file \fileloc{ENABLE_AMAZON_AUTO_BRIGHTNESS} to opt into Amazon ambient brightness; remove it to use manual brightness. In KOReader, open \menupath{Settings -> Frontlight} for manual brightness and warmth. Open \menupath{Settings -> Screen -> Auto warmth and night mode -> Activate} for time/sun-based warmth; tap the checked activation choice again to turn it off. Edge and two-finger gestures may also alter brightness or warmth; change them under \menupath{Settings -> Taps and gestures -> Gesture manager}.
...
For the PW2-to-PW5SE migration, the completed initial 249-PDF snapshot copied no sidecars during its PDF transfer. The later guarded apply reported 10 copied sidecar trees and 239 skips: one existing destination, 161 not inspected, 68 absent or ambiguous, and nine checksum/current-PDF mismatches. Candidate identity used current PDF size and KOReader partial MD5 rather than stale metadata checksums. An independent audit matched every file in all 10 copied trees to its PW2 source by size and SHA-256, found no owned temporary files, and both keep-awake guards restored their original \texttt{preventScreenSaver=0} value.

\emph{Zizhi Tongjian, Part 1} was the existing-destination skip and was not overwritten. The PW5SE records page 11 while the PW2 records page 4651; both report 18079 document pages, and the full PDF SHA-256 matches Nutstore, PW2, and PW5SE. Preserving PW5SE state is the safe default. Its explicit transactional replacement path is implemented and tested but has not run; replacement awaits closing the book in KOReader/file browser and a separate guarded run. \emph{Shiji} uses differently named or different-edition sidecars, so no match is guessed and none was copied.

The 249 count is an initial dated snapshot. As of 2026-08-13, the live Nutstore source contains 256 LinguaLeaf plus 10 PocketPolished PDFs (266 total). Later individually requested books were copied, but a fresh full-device 266-book reconciliation has not run, so only the original 249-book snapshot is claimed fully reconciled.
...
Latest application downloads: \url{https://github.com/lachlanchen/Kindle/releases/latest}

Original project website without redirection: \url{https://lachlanchen.github.io/Kindle/?stay=1}
\end{center}
...
\subsection{Recommended first connection: no-password Wi-Fi bootstrap}

\begin{enumerate}
  \item Download and open Kindle Book Sender from the website.
  \item In KOReader, stop SSH if it is already running, enable \textbf{Login without password (DANGEROUS)}, then start SSH again.
  \item Enter the address and port displayed by KOReader, such as \texttt{192.168.5.26:2222}. A hostname, IPv6 address, or another routed private subnet also works.
  \item Check \textbf{First connection: KOReader allows login without password}, then click \textbf{Connect / find Kindle}.
  \item The app creates a new RSA key for that computer, connects without a password once, installs only its public key, and verifies a second connection with the key.
\end{enumerate}

The app-generated private key stays in that computer's user application-data folder. A new computer can repeat the no-password bootstrap without installing Python, SSH tools, or a key manually. The app does not disable KOReader's no-password option; leaving it enabled is convenient but allows any reachable client to attempt root access.

\subsection{USB pairing fallback}

Exit KOReader first so the Kindle drive can appear. Connect USB, click \textbf{Pair over USB}, safely eject, reopen KOReader, and start SSH. If KOReader remains open and the drive does not appear, use no-password Wi-Fi bootstrap instead; USB alone cannot modify storage that the Kindle has not exported to the computer.

\subsection{Every wireless transfer after pairing}

\begin{enumerate}
  \item Put the Kindle and computer on the same trusted Wi-Fi, or on networks that can route to each other.
  \item Open KOReader, then start \textbf{Tools > SSH server}.
  \item In the app, click \textbf{Connect / find Kindle}.
  \item Drag books into the app or choose them with the file picker.
  \item Click \textbf{Send books}.
\end{enumerate}

The app always tests a supplied address first, including hostname, IPv4, IPv6, custom port, and routed subnet addresses. It then scans every active private IPv4 interface as a fallback. A candidate is accepted only after authentication and confirmation that \path{/mnt/us/koreader} exists. Failures distinguish DNS, routing, closed port, timeout, passwordless rejection, and key rejection. Books are placed in \path{/mnt/us/documents/Books}; SFTP is used when available and SCP is selected automatically as a fallback.
...
If discovery does not succeed, enter the exact address shown when KOReader SSH starts. Guest Wi-Fi and access-point client isolation can block two devices even when the Wi-Fi name is identical. On Windows, the app can request a UAC-approved outbound firewall rule, but that rule cannot repair router isolation or a missing route.

\section{Kindle Book Sender 1.3}

The desktop sender is available for Windows, macOS, and Linux. Version 1.3 adds an
11-language interface, automatic locale selection, a maximized responsive layout,
Kindle storage usage, remembered Kindle addresses, the last book folder, and a
persistent recent-transfer history.

On Windows, the packaged application maintains a per-user installed copy under
\path{%LOCALAPPDATA%/Programs/LazyingArt/Kindle Book Sender} and creates a Start
menu shortcut without administrator access. Use \textbf{Add to Start} or
\textbf{Add to taskbar} inside the application when needed. Windows 11 may block
automatic taskbar pinning; if so, right-click the running icon and choose
\textbf{Pin to taskbar}.

The \textbf{Guide \& downloads} button opens \url{https://lazying.art/eink}.
The English and Simplified Chinese printable guides remain available directly at:
...
\path{/mnt/us/_DISABLE_KOREADER_AUTOSTART} & PW5SE regular marker: standard autostart mode \\
\path{/mnt/us/_DISABLE_KOREADER_AUTOSTART_FRAMEWORK_STOP} & PW5SE regular marker: framework-stop autostart mode \\
\path{/mnt/us/DISABLE_KOREADER_AUTOSTART} & PW5SE regular marker: autostart disabled \\
\bottomrule
\end{longtable}
...
Hold the power button until the device restarts; this can take roughly 40 seconds. The installed PW5SE v2 job is configured to launch KOReader once unless the active recovery marker has been set, but its first reboot test is still pending. Choose \menupath{Exit KOReader} for a clean return; the job contains no respawn behavior during that boot.

\subsection{Recovery marker}

For the installed PW5SE v2 deployment, disable autostart by atomically renaming the single enabled regular marker present at the top level of the drive. Its exact name is one of:

\begin{Verbatim}[fontsize=\small]
_DISABLE_KOREADER_AUTOSTART
_DISABLE_KOREADER_AUTOSTART_FRAMEWORK_STOP
\end{Verbatim}

Framework-stop is currently selected, so the second name is the expected current file. Rename that one present file to the exact disabled name:

\begin{Verbatim}[fontsize=\small]
DISABLE_KOREADER_AUTOSTART
\end{Verbatim}

If the disabled name is already the sole regular marker, autostart is already disabled and no rename is needed. If no marker exists, multiple marker names exist, or the marker is not a regular file, stop and use the audited manager. Safely eject, disconnect USB data, and reboot. Re-enable or select an enabled mode only through the audited manager, which verifies the production job hash before changing the marker name.

\section{Dedicated handoff SSH key}

This package contains an RSA 3072-bit key pair made only for the owner's paired
KOReader Kindles:
...
The public key must be present in KOReader's SSH authorization file before this handoff key can connect. Verify it or run the included USB installer. The private key is not a default Windows SSH key; supplied commands select it explicitly with \texttt{-i}.

\subsection{Important warning}

\begin{center}
\fbox{\parbox{0.90\textwidth}{
\textbf{This private key is intentionally exposed in this PDF. Use it only for the owner's paired KOReader Kindles. Never authorize it on Windows, Linux computers, servers, routers, GitHub, cloud accounts, an unrelated Kindle, or any other device. Anyone holding this PDF can access a paired Kindle whenever KOReader SSH is running and reachable.}
}}
\end{center}
...
Need native Kindle & Exit KOReader; the installed PW5SE job contains no respawn behavior during that boot. For the next boot, activate the recovery marker by rename. \\
\bottomrule
\end{longtable}
