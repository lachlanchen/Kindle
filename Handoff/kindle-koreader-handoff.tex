...
Autostart must always be optional and device-specific. The repository's accepted PW5SE deployment uses two persistent marker names at the visible USB root. The PW4 named on this guide's cover is a separate firmware/package family; do not copy the PW5SE root job to it.

\begin{tabularx}{\textwidth}{>{\ttfamily}p{0.43\textwidth} X}
\toprule
\_DISABLE\_KOREADER\_AUTOSTART & Parked regular file: autostart is enabled. \\
DISABLE\_KOREADER\_AUTOSTART & Active regular file: autostart is disabled and the native interface remains. \\
\bottomrule
\end{tabularx}

The state change is rename-only: never delete either marker. Disable by atomically renaming the parked name to the active name. Enable only through the audited manager, which checks the exact installed job hash before performing the reverse rename. The job gives the native system a recovery window, applies final safety gates, launches only once, and never respawns KOReader. Therefore choosing \menupath{Exit KOReader} returns to the normal interface for the rest of that boot.

\begin{warningbox}
\textbf{Accepted PW5SE state.} The production job SHA-256 is \path{2de0232b971926b7e70d913a27ba76168ed69760504ae2a90947e4402e7e5828}. With USB data disconnected, reboot traced \texttt{started -> native-ready -> launching}; the job was \texttt{start/running}, KOReader was running, and Wi-Fi/SSH returned. The parked \fileloc{_DISABLE_KOREADER_AUTOSTART} regular file is present and the active name is absent. The root filesystem is read-only and \texttt{preventScreenSaver=0}. Exiting KOReader left the job \texttt{stop/waiting} with no reader after at least 45 seconds, proving no respawn.
\end{warningbox}
...
The 249 count is an initial dated snapshot. A later Nutstore addition, \texttt{Giving Up the Gun...｜黑白.pdf}, increased the desired corpus to 250 (240 LinguaLeaf plus 10 PocketPolished); that new book is pending and is not yet claimed transferred.
...
\path{/mnt/us/_DISABLE_KOREADER_AUTOSTART} & PW5SE parked regular marker: enabled \\
\path{/mnt/us/DISABLE_KOREADER_AUTOSTART} & PW5SE active regular marker: disabled \\
\bottomrule
\end{longtable}
...
Hold the power button until the device restarts; this can take roughly 40 seconds. On the accepted PW5SE job, a reboot launches KOReader once again unless the active recovery marker has been set. Choose \menupath{Exit KOReader} for a clean return without respawn during the current boot.

\subsection{Recovery marker}

For the accepted PW5SE deployment, disable autostart by renaming this parked regular file at the top level of the drive:

\begin{Verbatim}[fontsize=\small]
_DISABLE_KOREADER_AUTOSTART
\end{Verbatim}

to the exact active name:

\begin{Verbatim}[fontsize=\small]
DISABLE_KOREADER_AUTOSTART
\end{Verbatim}

Do not delete or create marker files, and do not guess if the parked file is missing or both names exist. Safely eject, disconnect USB data, and reboot. Re-enable only through the audited manager, which verifies the production job hash before reversing the rename.
...
Need native Kindle & Exit KOReader; the accepted PW5SE job will not respawn it during that boot. For the next boot, activate the recovery marker by rename. \\
\bottomrule
\end{longtable}
